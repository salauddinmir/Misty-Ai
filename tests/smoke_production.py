...
def main() -> int:
    ...
    check(
        ...
        f"{resp3.status_code} content-type={resp3.headers.get('content-type')}; "
        f"events=thinking+token+done={done_present and tokens_present}",
    )
    ...
    candidates.extend(
        value["last_autonomous_tick"]
        for value in data5.values()
        if isinstance(value, dict) and "last_autonomous_tick" in value
    )
    candidates = [candidate for candidate in candidates if candidate]
    metric_keys = ("tick_index", "evidence_budget", "evidence_count", "elapsed_ms", "outcome")
    results = [[k for k in metric_keys if k not in tick] for tick in candidates]
    ...
    check(
        ...
        f"status={resp5.status_code}; top-level keys={sorted(data5.keys())[:12]}; missing keys={missing}; "
        f"tick={json.dumps(best_tick, ensure_ascii=False)[:300]}",
    )
